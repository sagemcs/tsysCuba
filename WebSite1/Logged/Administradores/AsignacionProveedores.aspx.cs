...
public partial class AsignacionProveedores : System.Web.UI.Page
{
    ...
    protected void Datos2(string ID)
    {
        ...
        try
        {
            ...
            string Company = HttpContext.Current.Session["IDCompany"].ToString();
            SQL = "";
            SQL = "  Select b.UserName as Actual,b.userkey as UKey From vendors a inner join Users b on a.Superior= b.UserKey ";
            SQL = SQL + " Where a.VendName = '" + ID + "' And a.CompanyID = '" + Company + "'";
            using (SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["PortalConnection"].ToString()))
            {
                SqlCommand cmd = new SqlCommand(SQL, conn);
