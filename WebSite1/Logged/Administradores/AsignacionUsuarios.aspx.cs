...
public partial class AsignacionUsuarios : System.Web.UI.Page
{
    ...
    protected void Datos(string ID)
    {
        try
        {
            ...
            SQL = " Select a.UserName as Proveedor,b.Descripcion as Estatus,c.UserID as Useri ";
            SQL = SQL + " From Users a ";
            SQL = SQL + " inner join StatusUsers b on a.Status = b.Status ";
            SQL = SQL + " inner join Users c on a.Superior = c.UserKey ";
    ...
    protected void Datos2(string ID)
    {
        ...
        try
        {
            ...
            string Company = HttpContext.Current.Session["IDCompany"].ToString();
            SQL = "";
            SQL = "  Select a.UserID as Correo From users a ";
            SQL = SQL + " Where a.UserName = '" + ID + "'";
            using (SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["PortalConnection"].ToString()))
            {
                SqlCommand cmd = new SqlCommand(SQL, conn);
                cmd.CommandType = CommandType.Text;
        ...
        try
        {
            ...
            string Company = HttpContext.Current.Session["IDCompany"].ToString();
            SQL = "";
            SQL = "  Select b.UserName as Actual,b.userkey as UKey From Users a inner join Users b on a.Superior= b.UserKey ";
            SQL = SQL + " Where a.UserName = '" + ID + "'";
            using (SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["PortalConnection"].ToString()))
            {
                SqlCommand cmd = new SqlCommand(SQL, conn);
