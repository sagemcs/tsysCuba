...
public partial class Logged_Administradores_EditReembolso : System.Web.UI.Page
{
    ...
    private int Update_Reembolso(int expense_id, DateTime fecha_gasto, decimal importe_gasto, int userkey, string companyId, List<ExpenseDetailDTO> expenseDetails, string motivo_gasto)
    {
        ...
        try
        {
            ...
            using (SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["PortalConnection"].ToString()))
            {
                ...
                xml_files.ForEach((x) => {x.ExpenseId = id;  if (!Doc_Tools.CheckFileExist(x)) {Doc_Tools.SaveFile(x); }});
                pdf_files.ForEach((x) => {x.ExpenseId = id;  if (!Doc_Tools.CheckFileExist(x)) {Doc_Tools.SaveFile(x); }});
                voucher_files.ForEach((x) => {x.ExpenseId = id;  if (!Doc_Tools.CheckFileExist(x)) {Doc_Tools.SaveFile(x); }});                
            }
    ...
    protected void btn_validar_Click(object sender, EventArgs e)
    {
        ...
        else { if(expense.FileNameXml.Count == 0) { xml = true; } }
        ...
        else { if (expense.FileNamePdf.Count == 0) { pdf = true; } }
        ...
        else { if (expense.FileNamePdfVoucher.Count == 0) { voucher = true; } }
