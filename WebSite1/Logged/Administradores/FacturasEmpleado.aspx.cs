...
public partial class FacturasEmpleado : System.Web.UI.Page
{
    ...
    protected void GridView2_RowCommand(object sender, EventArgs e)
    {
        try
        {
            ...
            TextBox txtPrecioUnitario = (TextBox)GridView1.Rows[index].FindControl("UnitC");
    ...
    protected void Btn_Buscar(object sender, EventArgs e)
    {
        try
        {
            if (NoOc.Text != "")
            {
                ...
                if (SelProv.SelectedItem.ToString() == "") 
                {
                    string Eror = "Seleccione Proveedor";
                    string Caja = "B6";
                    Label4.Text = Eror;
                ...
                SQL = SQL + " where tranNo = '" + NoOc.Text + "' AND a.CompanyID = '" + Company + "' AND c.vendname ='" + SelProv.SelectedItem.ToString() + "' ";
                ...
                if (Filas == 0)
                {
                    ...
                    Datos.Visible = false;
                    string Caja = "B6";
                    Label4.Text = "Orden de Compra no Pertenece al Proveedor Seleccionado, Verificalo. ";
                    ScriptManager.RegisterStartupScript(this.Page, this.Page.GetType(), "ramdomtext", "alert(" + Caja + ");", true);
                }
                else if (Filas >= 1)
                {
                    ...
                    //SQL = SQL + " inner join timitem a on d.itemkey = a.itemkey ";
                    //SQL = SQL + " inner join timItemDescription b on b.itemkey = a.itemkey ";
                    //SQL = SQL + " where tranNo = '" + NoOc.Text + "' AND a.CompanyID = '" + Company + "' AND c.vendname ='" + SelProv.SelectedItem.ToString() + "' AND CAST(Round((l.qtyord - l.qtyInvcd),4) AS DECIMAL(20,4)) > 0 ";

                    SQL = "";
                    SQL = " select d.POLineNo as PoLine,c.vendname as Proveedor,CAST(Round((l.qtyord - l.qtyInvcd),4) AS DECIMAL(20,4)) as Cantidad, a.ItemID As Art ,b.ShortDesc , d.UnitCost , d.ExtAmt, (l.ExtAmt -l.AmtInvcd) as restante, l.qtyord as cantOrdenada";
                    SQL = SQL + " from tpopurchorder o ";
                    SQL = SQL + " join tapvendor c on o.vendkey = c.vendkey ";
                    SQL = SQL + " inner join tpoPOLine AS d on o.POKey = d.POKey ";
                    SQL = SQL + " inner join tpoPOLinedist l on l.POLineKey = d.POLineKey ";
                    SQL = SQL + " inner join timitem a on d.itemkey = a.itemkey ";
                    SQL = SQL + " inner join timItemDescription b on b.itemkey = a.itemkey ";
                    SQL = SQL + " where tranNo = '" + NoOc.Text + "' AND a.CompanyID = '" + Company + "' AND c.vendname ='" + SelProv.SelectedItem.ToString() + "' AND ((CAST(Round((l.qtyord - l.qtyInvcd),4) AS DECIMAL(20,4)) > 0 OR (l.ExtAmt -l.AmtInvcd) > 0))";
                    ...
                    using (SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["ConnectionString"].ToString()))
                    {
                        ...
                        if (Filas == 0)
                        {
                            ...
                            Datos.Visible = false;
                            string Caja = "B6";
                            Label4.Text = "Orden de Compra no tiene Partidas Pendientes por Cargar";
                            ScriptManager.RegisterStartupScript(this.Page, this.Page.GetType(), "ramdomtext", "alert(" + Caja + ");", true);
                        }
                        else if (Filas >= 1)
                        {
                            ...
                            if (Adenda == true) { BXml.Visible = true; }
            ...
            {
               string Caja = "B10";
                Label4.Text = "Numero de orden requerido, favor de verificar e intentar nuevamente.";
               ScriptManager.RegisterStartupScript(this.Page, this.Page.GetType(), "ramdomtext", "alert(" + Caja + ");", true);
            }
        }
        catch (Exception a)
    ...
    protected void llenatabla(System.Data.DataTable dts)
    {
        try
        {
            ...
            Decimal[] precioUnitario = new Decimal[dts.Rows.Count];
            foreach (DataRow row in dts.Rows)
            {
                DataRow dr = dt.NewRow();
                Decimal cantOrdenada = Convert.ToDecimal(HttpUtility.HtmlDecode(row["cantOrdenada"].ToString()));
                Decimal subTotal = Convert.ToDecimal(HttpUtility.HtmlDecode(row["restante"].ToString()));
                Decimal cantidadd = 0;

                if (cantOrdenada == 1 && subTotal > 0)
                {
                    cantidad[i] = HttpUtility.HtmlDecode(row["cantOrdenada"].ToString());
                    cantidadd = Convert.ToDecimal(HttpUtility.HtmlDecode(row["cantOrdenada"].ToString()));
                }
                else 
                {
                    cantidad[i] = HttpUtility.HtmlDecode(row["Cantidad"].ToString());
                    cantidadd = Convert.ToDecimal(HttpUtility.HtmlDecode(row["Cantidad"].ToString()));
                }

                precioUnitario[i] = Convert.ToDecimal(HttpUtility.HtmlDecode(row["restante"].ToString()));
                decimal preciouni = Convert.ToDecimal(HttpUtility.HtmlDecode(row["restante"].ToString()));
                ...
                dr["ExtAmt"] = Math.Round(Convert.ToDecimal(montoline), 2).ToString("0.00");
                dt.Rows.Add(dr);
                i++;
            }
            ...
            foreach (GridViewRow gvr in GridView1.Rows)
            {
                ...
                cantidades.Text = Math.Round(Convert.ToDecimal(cantidad[i].ToString()),2).ToString("0.00");
            ...
            foreach (GridViewRow gvr in GridView1.Rows)
            {
                TextBox precioUnit = (TextBox)gvr.Cells[1].FindControl("UnitC");
                //precioUnit.Text = Decimal.Round(precioUnitario[i],2).ToString();
                precioUnit.Text = Math.Round(Convert.ToDecimal(precioUnitario[i].ToString()),2).ToString("0.00");
                precioUnit.Enabled = false;
                i++;
            }
        }
        catch (Exception ex)
    ...
    protected int validaDocuments()
    {
        ...
        try
        {
            ...
            string CompanyID = HttpContext.Current.Session["IDCompany"].ToString();
            string vendor = SelProv.SelectedItem.ToString();
            SqlConnection sqlConnection1 = new SqlConnection();
            sqlConnection1 = SqlConnectionDB("PortalConnection");
            string sSQL;
            sqlConnection1.Open();

            string consulta = " Select Count(*) From Documents a inner join Vendors b on a.VendorKey = b.VendorKey ";
            consulta += " Where a.Status = 1 AND b.VendName = @OC AND a.CompanyID = @Company";
            sSQL = @"" + consulta;

            using (var sqlQuery = new SqlCommand(sSQL, sqlConnection1))
            {
                sqlQuery.Parameters.AddWithValue("@OC", vendor);
                sqlQuery.Parameters.AddWithValue("@Company", CompanyID);
                ...
                if (sqlQueryResult != null)
                {
                   while (sqlQueryResult.Read())
                   {
                       CantidadO = Convert.ToInt32(sqlQueryResult.GetValue(0));
                   }
                }
            }
            sqlConnection1.Close();
    ...
    protected int validaFolio(string Folio, string OC)
    {
        int CantidadO = 0;
        try
        {
            ...
            string consulta = " Select Count(*) From invoice a inner join Vendors b on a.VendorKey = b.VendorKey ";
            consulta += " Where a.Folio = @Folio AND NodeOC = @OC AND b.VendName = @vendor AND a.CompanyID = @Company";
            sSQL = @"" + consulta;

            using (var sqlQuery = new SqlCommand(sSQL, sqlConnection1))
            {
                sqlQuery.Parameters.AddWithValue("@vendor", vendor);
                sqlQuery.Parameters.AddWithValue("@Company", CompanyID);
                sqlQuery.Parameters.AddWithValue("@OC", OC);
                sqlQuery.Parameters.AddWithValue("@Folio", Folio);
    ...
    protected void Sage()
    {
        try
        {
            ...
            bool Adenda = Global.VerTipoProv(VendKey);
            if (Adenda == true) { Tipo = "A"; }
            else { bool Extranjero = Global.VerTipoProvE(VendKey); if (Extranjero == true) { Tipo = "E"; } }
            ...
            if (validaDocuments() > 0)
            {
                Label4.Text = "El Proveedor aun tiene Documentos pendientes de Cargar, No se puede realizar la Carga de Facturas";
                ScriptManager.RegisterStartupScript(this.Page, this.Page.GetType(), "ramdomtext", "alert(B6);", true);
                System.Threading.Thread.Sleep(1000);
                return;
            }

            if (validaFolio(FolioF.Text, NoOc.Text) > 0)
            {
                Label4.Text = "Ya se ha cargado una factura con ese número de folio para la de Orden de Compra seleccionada.";
                ScriptManager.RegisterStartupScript(this.Page, this.Page.GetType(), "ramdomtext", "alert(B6);", true);
                System.Threading.Thread.Sleep(1000);
                return;
            }
            ...
            if (Tipo == "A")
            {
                carga = databaseFilePutDB2(FileUpload1.PostedFile.InputStream, FileUpload2.PostedFile.InputStream, FileUpload3.PostedFile.InputStream, VendKey, 0, CompanyID);
                string error3 = HttpContext.Current.Session["Error"].ToString();
                extranjero = false;
            }
            else
            {
                carga = databaseFilePutDB(FileUpload2.PostedFile.InputStream, FileUpload3.PostedFile.InputStream, VendKey, 0, CompanyID);
                string error3 = HttpContext.Current.Session["Error"].ToString();
                extranjero = true;
            }
    ...
    private bool CargarXML(MemoryStream fs, int InvcFileKey)
    {
        try
        {
            ...
            Decimal ImpuestoImporteTrasladoXml = 0;
            Decimal SubtotalXml = 0;
            Decimal TotalXml = 0;
            ...
            if (Adenda == true)
            {
                ...
                try
                {
                    ...
                    try
                    {
                        ...
                        fecha = Facturas.Fecha.ToString();
                        ImpuestoImporteTrasladoXml = Facturas.Impuestos.TotalImpuestosTrasladados;
                        SubtotalXml = Facturas.SubTotal;
                        TotalXml = Facturas.Total;

                        Tuple<Decimal, Decimal, Decimal> resultCantidades = obtenerSumatoriaDeSubtotalImpuestoTotal();
                        Decimal subTotal = resultCantidades.Item1;
                        Decimal impuesto = resultCantidades.Item2;
                        Decimal total = resultCantidades.Item3;

                        if (SubtotalXml > subTotal)
                        {
                            ScriptManager.RegisterStartupScript(this.Page, this.Page.GetType(), "ramdomtext", "alert(B11);", true);
                            System.Threading.Thread.Sleep(1000);
                            return false;
                        }

                        if (ImpuestoImporteTrasladoXml > impuesto)
                        {
                            ScriptManager.RegisterStartupScript(this.Page, this.Page.GetType(), "ramdomtext", "alert(B12);", true);
                            System.Threading.Thread.Sleep(1000);
                            return false;
                        }
                    ...
                    catch (Exception ex)
                    {
                        try
                        {
                            ...
                            ImpuestoImporteTrasladoXml = Factura.Impuestos.TotalImpuestosTrasladados;
                            SubtotalXml = Factura.SubTotal;
                            TotalXml = Factura.Total;
            ...
            try
            {
                ...
                string consulta = "Select top 1 d.VendDBA,e.FedID,d.VendName,e.CompanyName,' ' as Folio,GETDATE(),c.CurrID ,c.STaxAmt,c.PurchAmt,c.TranAmt  ";
                consulta += " From tpoPurchOrder c";
                    ...
                    using (var sqlQueryResult = sqlQuery.ExecuteReader())
                        if (sqlQueryResult != null)
                        {
                            while (sqlQueryResult.Read())
                            {
                                ...
                                {
                                    FechaTransaccion = Convert.ToDateTime(fecha);
                                }
                                Moneda = Convert.ToString(sqlQueryResult.GetValue(6));

                                //Codigo provicional en lo que identificamos los impuestos de sage
                                ...
                                //ImpuestoImporteTraslado = impuesto;
                                //Subtotal = subTotal;
                                //Total = total;

                                if (ImpuestoImporteTrasladoXml == 0)
                                {
                                    ImpuestoImporteTraslado = impuesto;
                                }
                                else
                                {
                                    ImpuestoImporteTraslado = ImpuestoImporteTrasladoXml;
                                }
            ...
            else
            {
                try
                {
                    ...
                    String ordenCompra = NoOc.Text;
                    String companyID = HttpContext.Current.Session["IDCompany"].ToString();
                    int ordenCompraKey = 0;

                    SqlConnection sqlConnection2 = new SqlConnection();
                    sqlConnection2 = SqlConnectionDB("ConnectionString");
                    sqlConnection2.Open();

                    string consultaOrdenCompra = "SELECT POKey FROM tpoPurchOrder WHERE TranNo= @OC AND CompanyID= @CompanyID";

                    using (var sqlQuery = new SqlCommand(consultaOrdenCompra, sqlConnection2))
                    {
                        sqlQuery.Parameters.AddWithValue("@OC", ordenCompra);
                        ...
                        using (var sqlQueryResult = sqlQuery.ExecuteReader())
                            if (sqlQueryResult != null)
                            {
                                while (sqlQueryResult.Read())
                                {
                                    ordenCompraKey = Convert.ToInt32(sqlQueryResult["POKey"].ToString());
                                }
                            }
                    }
                    ...
                    foreach (GridViewRow gvr in GridView1.Rows)
                    {
                        ...
                        if (Checka.Checked == true) 
                        {
                            ...
                            TextBox txtPrecioUnitario = (TextBox)gvr.FindControl("UnitC");
                            ...
                            if (Convert.ToDecimal(txtPrecioUnitario.Text) > 0)
                            {
                                ...
                                string consulta = "SELECT timItem.ItemKey, timItem.ItemID, tpoPOLine.ItemKey, tpoPOLine.POLineNo, tpoPOLine.POLineKey, tpoPOLine.UnitCost, tpoPOLineDist.QtyOrd, tpoPOLine.ExtAmt, tpoPOLine.Description, tciSTaxCodeTran.CalcSTaxAmt, tciSTaxSubjClassDt.Rate, tpoPOLine.STaxClassKey, STaxClassID, STaxSchdID, tciSTaxTran.STaxSchdKey, tciSTaxSchedule.STaxSchdID, 0 as descuento";
                                consulta += " FROM tpoPOLine";
                                consulta += " INNER JOIN tpoPOLineDist on tpoPOLine.POLineKey = tpoPOLineDist.POLineKey";
                                consulta += " INNER JOIN timItem on tpoPOLine.ItemKey = timItem.ItemKey";
                                consulta += " INNER JOIN tciSTaxTran on tciSTaxTran.STaxTranKey = tpoPOLineDist.STaxTranKey";
                                consulta += " INNER JOIN tciSTaxClass on tpoPOLine.STaxClassKey = tciSTaxClass.STaxClassKey";
                                consulta += " INNER JOIN tciSTaxSchedule on tciSTaxTran.STaxSchdKey = tciSTaxSchedule.STaxSchdKey";
                                consulta += " INNER JOIN tciSTaxCodeTran ON  tciSTaxTran.STaxTranKey = tciSTaxCodeTran.STaxTranKey";
                                consulta += " INNER JOIN tciSTaxCode ON tciSTaxCodeTran.STaxCodeKey = tciSTaxCode.STaxCodeKey";
                                consulta += " INNER JOIN tciSTaxCodeCompany ON tciSTaxCode.STaxCodeKey = tciSTaxCodeCompany.STaxCodeKey";
                                consulta += " INNER JOIN tciSTaxSubjClass ON tciSTaxSubjClass.STaxClassKey = tpoPOLine.STaxClassKey AND tciSTaxSubjClass.STaxCodeKey = tciSTaxCodeTran.STaxCodeKey";
                                consulta += " INNER JOIN tciSTaxSubjClassDt ON tciSTaxSubjClass.STaxCodeKey = tciSTaxSubjClassDt.STaxCodeKey AND tciSTaxSubjClass.STaxClassKey = tciSTaxSubjClassDt.STaxClassKey";
                                consulta += " WHERE POKey = @POKey and tciSTaxCodeCompany.CompanyID = @Company and tpoPOLine.POLineNo = @Linea";
                                sSQL = @"" + consulta;

                                using (var sqlQuery = new SqlCommand(consulta, sqlConnection1))
                                {
                                    sqlQuery.Parameters.AddWithValue("@POKey", ordenCompraKey);
                                    sqlQuery.Parameters.AddWithValue("@Company", CompanyID);
                                    sqlQuery.Parameters.AddWithValue("@Linea", linea);
                                    ...
                                    using (var sqlQueryResult = sqlQuery.ExecuteReader())
                                        if (sqlQueryResult != null)
                                        {
                                            while (sqlQueryResult.Read())
                                            {
                                                ...
                                                Art = Convert.ToString(sqlQueryResult["ItemID"]);
                                                strDescr = Convert.ToString(sqlQueryResult["Description"]);
                                                UnitCost = Convert.ToDecimal(txtPrecioUnitario.Text);
                                                Cantidad = Convert.ToDecimal(canti.Text);
                                                Monto = Cantidad * Convert.ToDecimal(txtPrecioUnitario.Text);
                                                DescuentoItm = Convert.ToDecimal(sqlQueryResult["descuento"]);
                                                Partidas = Convert.ToInt32(sqlQueryResult["POLineNo"]);
                                                ...
                                                if (vkeydtl > 0 & resultado == 1)
                                                {
                                                    ...
                                                    Tuple<int, int> resutlDtlTaxT = InsertInvoiceDtlTaxDB(vkey, vkeydtl, (Monto * Convert.ToDecimal(sqlQueryResult["Rate"])), "002", Convert.ToDecimal(sqlQueryResult["Rate"]), "T", Status, UserKey, CompanyID, LogKey);
    ...
    private Tuple<Decimal,Decimal,Decimal> obtenerSumatoriaDeSubtotalImpuestoTotal()
    {
        Decimal subTotal = 0;
        Decimal impuesto = 0;
        Decimal subTotalSumatoria = 0;
        Decimal impuestoSumatoria = 0;
        Decimal total = 0;
        int ordenCompraKey = 0;
        String companyID = HttpContext.Current.Session["IDCompany"].ToString();
        String ordenCompra = NoOc.Text;
        ...
        try
        {
            ...
            foreach (GridViewRow gvr in GridView1.Rows)
            {
                ...
                {
                    string linea = HttpUtility.HtmlDecode(gvr.Cells[1].Text.ToString());
                    TextBox txtcantidad = (TextBox)gvr.Cells[2].FindControl("cant");
                    TextBox txtPrecioUnitario = (TextBox)gvr.Cells[2].FindControl("UnitC");
                    ...
                    if (Convert.ToDecimal(txtPrecioUnitario.Text) > 0)
                    {
                        ...
                        string consultaPartida = "SELECT tpoPOLine.ItemKey, tpoPOLine.POLineNo, tpoPOLine.POLineKey, tpoPOLine.UnitCost, tpoPOLineDist.QtyOrd, tpoPOLine.ExtAmt, tciSTaxCodeTran.CalcSTaxAmt, tciSTaxSubjClassDt.Rate, tpoPOLine.STaxClassKey, STaxClassID, STaxSchdID, tciSTaxTran.STaxSchdKey, tciSTaxSchedule.STaxSchdID";
                        consultaPartida += " FROM tpoPOLine";
                        consultaPartida += " INNER JOIN tpoPOLineDist on tpoPOLine.POLineKey = tpoPOLineDist.POLineKey";
                        consultaPartida += " INNER JOIN tciSTaxTran on tciSTaxTran.STaxTranKey = tpoPOLineDist.STaxTranKey";
                        consultaPartida += " INNER JOIN tciSTaxClass on tpoPOLine.STaxClassKey = tciSTaxClass.STaxClassKey";
                        consultaPartida += " INNER JOIN tciSTaxSchedule on tciSTaxTran.STaxSchdKey = tciSTaxSchedule.STaxSchdKey";
                        consultaPartida += " INNER JOIN tciSTaxCodeTran ON  tciSTaxTran.STaxTranKey = tciSTaxCodeTran.STaxTranKey";
                        consultaPartida += " INNER JOIN tciSTaxCode ON tciSTaxCodeTran.STaxCodeKey = tciSTaxCode.STaxCodeKey";
                        consultaPartida += " INNER JOIN tciSTaxCodeCompany ON tciSTaxCode.STaxCodeKey = tciSTaxCodeCompany.STaxCodeKey";
                        consultaPartida += " INNER JOIN tciSTaxSubjClass ON tciSTaxSubjClass.STaxClassKey = tpoPOLine.STaxClassKey AND tciSTaxSubjClass.STaxCodeKey = tciSTaxCodeTran.STaxCodeKey";
                        consultaPartida += " INNER JOIN tciSTaxSubjClassDt ON tciSTaxSubjClass.STaxCodeKey = tciSTaxSubjClassDt.STaxCodeKey AND tciSTaxSubjClass.STaxClassKey = tciSTaxSubjClassDt.STaxClassKey";
                        consultaPartida += " WHERE POKey = @POKey AND tciSTaxCodeCompany.CompanyID = @Company AND tpoPOLine.POLineNo = @Linea";

                        using (var sqlQuery = new SqlCommand(consultaPartida, sqlConnection1))
                        {
                            sqlQuery.Parameters.AddWithValue("@POKey", ordenCompraKey);
                            sqlQuery.Parameters.AddWithValue("@Company", companyID);
                            sqlQuery.Parameters.AddWithValue("@Linea", linea);
                            ...
                            using (var sqlQueryResult = sqlQuery.ExecuteReader())
                                if (sqlQueryResult != null)
                                {
                                    while (sqlQueryResult.Read())
                                    {
                                        //subTotal = Convert.ToDecimal(txtcantidad.Text) * Convert.ToDecimal(sqlQueryResult["UnitCost"].ToString());
                                        subTotal = Convert.ToDecimal(txtcantidad.Text) * Convert.ToDecimal(txtPrecioUnitario.Text);
                                        impuesto = subTotal * Convert.ToDecimal(sqlQueryResult["Rate"].ToString());
                                    }
                                }
                        }
                        subTotalSumatoria = subTotalSumatoria + subTotal;
                        impuestoSumatoria = impuestoSumatoria + impuesto;
                        sqlConnection1.Close();
                    }
                    else
                    {
                        ScriptManager.RegisterStartupScript(this.Page, this.Page.GetType(), "ramdomtext", "alert(B14);", true);
                        System.Threading.Thread.Sleep(1000);
                        break;
                    }
                }
            }
            total = subTotalSumatoria + impuestoSumatoria;
            var result = Tuple.Create<Decimal, Decimal, Decimal>(subTotalSumatoria, impuestoSumatoria, total);
            return result;
        }
        catch 
        {
            var result = Tuple.Create<Decimal, Decimal, Decimal>(0, 0, 0);
            return result;
        }
    }
    ...
    private int databaseFilePutDB2(Stream fs1,Stream fs2, Stream fs3, int VendorKey, int InvoiceKey, String CompanyID)
    {
        try
        {
    ...
    protected void TextBox1_TextChanged(object sender, EventArgs e)
    {
        try
        {
            ...
            int index = row.RowIndex;
            TextBox txtPrecioUnitario = (TextBox)GridView1.Rows[index].FindControl("UnitC");
            TextBox txtCantidad = (TextBox)GridView1.Rows[index].FindControl("cant");
            decimal cantidad = Convert.ToDecimal(txtCantidad.Text);
            decimal precioUnitario = Convert.ToDecimal(txtPrecioUnitario.Text);
            if (cantidad > 0)
            {
                //decimal costo = Convert.ToDecimal(GridView1.Rows[index].Cells[7].Text);
                GridView1.Rows[index].Cells[8].Text = Math.Round(Convert.ToDecimal(cantidad * precioUnitario), 2).ToString("0.00");
            }
        }
        catch (Exception ex)
        {
