...
//PANTALLA PARA GASTOS MEDICOS MENORES DE EMPLEADOS
...
public partial class Logged_Administradores_GastosMedicosMenoresEmpleados : System.Web.UI.Page
{
    #region Variables
    ...
    private int WriteToDb(DateTime fecha_gasto, decimal importe_gasto, int userkey, string companyId, List<ExpenseDetailDTO> expenseDetails, int status)
    {
        ...
        {  
            ...
            using (SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["PortalConnection"].ToString()))
            {
                ...
                cmd.CommandText = "Insert Into MinorMedicalExpense (Date, Amount, CreateDate, UpdateDate, UpdateUserKey, CompanyId, Status, ApprovalLevel) VALUES (@Date, @Amount,  @CreateDate, @UpdateDate, @UpdateUserKey, @CompanyId, @Status, @ApprovalLevel); SELECT SCOPE_IDENTITY();";
                ...
                {
                    cmd.CommandText = "INSERT INTO MinorMedicalExpenseDetail (MinorMedicalExpenseId,ItemKey,Qty,UnitCost,Amount,CreateDate,UpdateDate,CreateUser,CompanyId,STaxCodeKey,TaxAmount) VALUES (@_MinorMedicalExpenseId,@_ItemKey,@_Qty,@_UnitCost,@_Amount,@_CreateDate,@_UpdateDate,@_CreateUser,@_CompanyId, @_STaxCodeKey, @_TaxAmount); SELECT SCOPE_IDENTITY();";
                    cmd.Parameters.Add("@_MinorMedicalExpenseId", SqlDbType.Int).Value = id;
        ...
        catch (Exception ex)
        {
            if (id > 0)
            {
                Doc_Tools.DeleteExpenseOnFail(Doc_Tools.DocumentType.MinorMedicalExpense, id);
                Doc_Tools.DeleteDetailOnFail(Doc_Tools.DocumentType.MinorMedicalExpense, id);
                Doc_Tools.DeleteFile(Doc_Tools.DocumentType.MinorMedicalExpense, id);
            }
            LogError(pLogKey, pUserKey, "Insertar-Pago-GastoMedicoMenor-Empleados:Insertar-Pago", ex.Message, pCompanyID);
            string err;
            err = ex.Message;
            HttpContext.Current.Session["Error"] = err;
            return -1;
    ...
    private List<MinorMedicalExpenseDTO> ReadFromDb(int user_id)
    {
    ...
    private bool Check_Exist(int userKey, decimal importe)
    {
        ...
        {
            SqlCommand cmd = conn.CreateCommand();
            cmd.CommandText = "SELECT count(*) FROM MinorMedicalExpense where UpdateUserKey = @UpdateUserKey and Amount = @Amount and CreateDate = @CreateDate";
            cmd.Parameters.Add("@Amount", SqlDbType.Decimal).Value = importe;
            cmd.Parameters.Add("@UpdateUserKey", SqlDbType.Int).Value = userKey;
            cmd.Parameters.Add("@CreateDate", SqlDbType.Date).Value = DateTime.Today.Date;
            cmd.Connection.Open();
    ...
    private void Sage()
    {
        ...
        if (is_valid)
        {
            ...
            int carga = WriteToDb(fecha_gasto, importe_gasto, pUserKey, pCompanyID, lista_detalles, status);
            ...
            btnFinalizar.Enabled = (bool)HttpContext.Current.Session["terminar_comprobacion"];
            Doc_Tools.EnviarCorreo(Doc_Tools.DocumentType.MinorMedicalExpense, pUserKey, 1, Doc_Tools.NotificationType.Revision, pUserKey);
        }
        else
        {
            btnSage.Enabled = false;
    ...
    {
        HttpContext.Current.Session["MinorMedicalExpense"] = null;
    ...
    private MinorMedicalExpenseDTO LoadMedicalExpenseById(int expense_id, int user_id)
    {
        var expense = new MinorMedicalExpenseDTO();
        using (SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["PortalConnection"].ToString()))
        {
            SqlCommand cmd = conn.CreateCommand();
            cmd.CommandText = "SELECT MinorMedicalExpenseId , Date ,Amount, Status FROM MinorMedicalExpense where UpdateUserKey = @UpdateUserKey and MinorMedicalExpenseId = @MinorMedicalExpenseId and Status = 1;";
            cmd.Parameters.Add("@UpdateUserKey", SqlDbType.Int).Value = user_id;
    ...
    protected void btn_additem_Click(object sender, EventArgs e)
    {
        ...
        if (tbx_cantidad.Text.Any(x => !char.IsDigit(x)))
        {
    ...
    protected void btn_validar_Click(object sender, EventArgs e)
    {
        ...
        {            
            tipo = "error";
            Msj = Doc_Tools.get_msg().FirstOrDefault(x => x.Key == "MB36").Value;         
            ScriptManager.RegisterStartupScript(this.Page, this.Page.GetType(), "ramdomtext", "alertme('" + titulo + "','" + Msj + "','" + tipo + "');", true);
            return;
        }     
