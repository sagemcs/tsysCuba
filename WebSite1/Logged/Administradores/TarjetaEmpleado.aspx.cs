...
public partial class Logged_Administradores_TarjetaEmpleado : System.Web.UI.Page
{
    ...
    private void Sage()
    {
        ...
        if (is_valid)
        {
            ...
            btnFinalizar.Enabled = (bool)HttpContext.Current.Session["terminar_comprobacion"];
            Doc_Tools.EnviarCorreo(Doc_Tools.DocumentType.CorporateCard, pUserKey, 1, Doc_Tools.NotificationType.Revision, pUserKey);
            GvItems.DataBind();
        }
        else 
        {
    ...
    protected void btn_comprobacion_Command(object sender, CommandEventArgs e)
    {
        ...
        {
            int card_id = int.Parse(row.Cells[0].Text);
            HttpContext.Current.Session["DocKey"] = card_id;
