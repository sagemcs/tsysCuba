...
//Revision de actualización: Cesilio Hernández López
...
public partial class Factura1 : System.Web.UI.Page
{
    ...
    protected void Sage()
    {
        try
        {
            ...
            string CompanyID = HttpContext.Current.Session["IDCompany"].ToString();
            int carga = databaseFilePutDB(FileUpload1.PostedFile.InputStream,FileUpload2.PostedFile.InputStream, FileUpload3.PostedFile.InputStream, pVendKey, 0, CompanyID);
            ...
            if (Adenda == true)
            {
                ...
                try
                {
                    ...
                    try
                    {
                        ...
                        total = Facturas.Total.ToString();
                        subtotal = Facturas.SubTotal.ToString();
                        impuesto = Facturas.Impuestos.TotalImpuestosTrasladados.ToString();
                    }
                    catch (Exception ex)
                    {
                        try
                        {
                            ...
                            impuesto = Factura.Impuestos.TotalImpuestosTrasladados.ToString();
                        }
                        catch (Exception exs)
                        {
                            //LOG Err
                    ...
                    using (var sqlQueryResult = sqlQuery.ExecuteReader())
                        if (sqlQueryResult != null)
                        {
                            while (sqlQueryResult.Read())
                            {
                                ...
                                {
                                    ImpuestoImporteTraslado = Convert.ToDecimal(impuesto);
                                }
    ...
    private int databaseFilePutDB(Stream fs1,Stream fs2, Stream fs3, int VendorKey, int InvoiceKey, String CompanyID)
    {
        try
        {
